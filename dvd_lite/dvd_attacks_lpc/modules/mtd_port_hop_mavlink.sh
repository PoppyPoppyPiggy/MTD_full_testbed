...
: "${CUTOVER_BLOCK_MS:=400}"# iptables 없을 때 인터페이스 전체 차단(모사) 시간(ms)
...
if_netem_block(){
  local ms="$1"
  "$BASE/sh_core/netem.sh" >/dev/null 2>&1 || true
...
main(){
  ...
  log "[mtd_port_hop_mavlink] new=$NEW_PORT -> old=$OLD_PORT grace=${GRACE}s drop_old=$DROP_OLD"
  ...
  if [ "${ALLOW_REAL_EFFECTS:-0}" -eq 1 ]; then
    if _have iptables; then
      ...
      command -v socat >/dev/null 2>&1 || docker exec -u 0 "$DVD_C_GCS" bash -lc 'apt-get update && apt-get install -y socat'
      socat_start "$NEW_PORT" "$OLD_PORT"
      bus_emit "mtd" "action=port_hop_socat new=$NEW_PORT old=$OLD_PORT target=$DVD_C_GCS"
    fi
  fi
  ...
  if [ "${ALLOW_REAL_EFFECTS:-0}" -eq 1 ] && [ "$DROP_OLD" = "1" ]; then
    if _have iptables; then
      ...
      # 포트 단위 드롭 불가 → 짧은 전체 차단으로 컷오버 모사
      if_netem_block "$CUTOVER_BLOCK_MS"
    fi
    bus_emit "mtd" "action=port_hop_cutover old=$OLD_PORT new=$NEW_PORT drop_old=1"
