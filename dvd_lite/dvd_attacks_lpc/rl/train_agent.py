...
# 최종 업데이트: v5.2 - CUDA/AMP 최적화, 버그픽스, 완전 자립 실행
...
class MtdGameEnv(gym.Env):
    ...
    def step(self, action):
        if self.training_agent == 'mtd':
            ...
            seeker_action, _ = self.seeker_agent.policy.act(torch.as_tensor(obs_seeker, dtype=torch.float32, device=DEVICE))
            seeker_action = int(seeker_action.item())
        else:
            seeker_action = int(action)
            obs_defender = self._get_defender_obs()
            defender_action, _ = self.defender_agent.policy.act(torch.as_tensor(obs_defender, dtype=torch.float32, device=DEVICE))
            defender_action = int(defender_action.item())
        ...
        elif defender_action == 4 and np.sum(self.attack_log) > 0:
...
class ICM(nn.Module):
    def __init__(self, state_dim, action_dim, lr=0.0003):
        ...
        self.inverse_model  = nn.Sequential(nn.Linear(self.feature_size*2, 128), nn.ELU(), nn.Linear(128, action_dim))
        self.forward_model  = nn.Sequential(nn.Linear(self.feature_size + action_dim, 128), nn.ELU(), nn.Linear(128, self.feature_size))
        self.optimizer = optim.Adam(self.parameters(), lr=lr)
        self.use_amp = (DEVICE.type == "cuda")
        self.scaler = torch.cuda.amp.GradScaler(enabled=self.use_amp)

    @torch.no_grad()
    def get_intrinsic_reward(self, state, next_state, action_onehot):
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            s  = self.feature_encoder(state)
            ns = self.feature_encoder(next_state)
            pred_ns = self.forward_model(torch.cat([s, action_onehot], dim=1))
            ir = 0.5 * (pred_ns - ns).pow(2).mean(dim=1)
        return ir
    ...
    def update(self, state, next_state, action_onehot):
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            ns = self.feature_encoder(next_state)
            pred_ns = self.forward_model(torch.cat([s, action_onehot], dim=1))
            forward_loss = 0.5 * (pred_ns - ns.detach()).pow(2).mean()
            pred_a = self.inverse_model(torch.cat([s, ns], dim=1))
            inverse_loss = nn.CrossEntropyLoss()(pred_a, action_onehot.argmax(dim=1))
...
class PPOAgent:
    def __init__(self, state_dim, action_dim, lr, ppo_epochs, use_icm=False, icm_config=None):
        ...
        if hasattr(torch, "compile"):
            try:
                self.policy = torch.compile(self.policy, fullgraph=False, mode="max-autotune")
                self.policy_old = torch.compile(self.policy_old, fullgraph=False, mode="max-autotune")
            except Exception:
                pass
        ...
        if self.use_icm:
            ...
            self.intrinsic_reward_weight = float(icm_config['intrinsic_reward_weight'])
        else:
            self.icm = None
            self.intrinsic_reward_weight = 0.0
        self.use_amp = (DEVICE.type == "cuda")
        self.scaler = torch.cuda.amp.GradScaler(enabled=self.use_amp)
    ...
    def update(self):
        ...
        import numpy as _np
        old_states = torch.from_numpy(_np.stack(self.buffer.states)).float().to(DEVICE, non_blocking=True)
        old_next_states = torch.from_numpy(_np.stack(self.buffer.next_states)).float().to(DEVICE, non_blocking=True)
        old_actions = torch.tensor(self.buffer.actions, dtype=torch.long).to(DEVICE, non_blocking=True)
        old_logprobs = torch.stack(self.buffer.logprobs).to(DEVICE, non_blocking=True)
...
def main():
    print("="*60)
    print("    게임 이론, 전략적 보상, 내재적 호기심 기반 학습 (v5.2)    ")
    ...
    time_step = 0
    i_episode = 0
    total_timesteps = int(cfg_train['total_timesteps'])
    update_timestep = int(cfg_train['update_timestep'])
    ...
    while time_step < total_timesteps:
        ...
        while not done and time_step < total_timesteps:
            ...
            if mtd_agent.use_icm:
                a_one = nn.functional.one_hot(torch.tensor(mtd_action, device=DEVICE), num_classes=mtd_action_dim).float()
                s     = torch.as_tensor(mtd_obs,      dtype=torch.float32, device=DEVICE).unsqueeze(0)
                s_np1 = torch.as_tensor(next_mtd_obs, dtype=torch.float32, device=DEVICE).unsqueeze(0)
                intrinsic = mtd_agent.icm.get_intrinsic_reward(s, s_np1, a_one.unsqueeze(0))
        ...
        if i_episode % 50 == 0:
            print(f"Episode {i_episode}, Timestep {time_step}/{total_timesteps}")
