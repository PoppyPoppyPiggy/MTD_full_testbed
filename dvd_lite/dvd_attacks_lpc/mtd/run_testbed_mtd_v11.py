...
class ExperimentConfig:
    ...
    n_episodes_per_seed: int = 50        # 시드당 에피소드 수
    max_episode_steps: int = 200         # 최대 스텝
    attacker_levels: List[int] = field(default_factory=lambda: [0, 1, 2, 3, 4])  # L0-L4
    
    # 시간 설정
    step_duration: float = 0.5           # 스텝당 시간 (초) - 0.5초로 단축
    time_compression: float = 10.0       # 시간 압축 비율
...
class MTDTestbedV10:
    ...
    def _plot_rl_cost_optimization(self):
        ...
        for seed_idx in range(len(rl_results.seed_results)):
            seed_data = rl_results.seed_results[seed_idx]
            
            for ep_idx, metrics in enumerate(seed_data.episode_metrics):
                episode_costs.append(metrics.get('cost', 0.0))
                episode_des.append(metrics.get('des', 0.0))
                episode_rewards.append(metrics.get('reward', 0.0))
                episode_numbers.append(ep_idx + seed_idx * self.config.n_episodes_per_seed)
        ...
        for i in range(len(episode_costs)):
            ...
            reward_smooth.append(np.mean(episode_rewards[start:end]))
