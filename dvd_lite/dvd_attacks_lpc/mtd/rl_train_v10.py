...
def main():
    ...
    parser.add_argument('--steps', type=int, default=150, help='Max steps per episode')  # 논문 일치
    ...
    # 디렉토리 생성 (에러 처리 추가)
    for directory in [args.output_dir, args.log_dir, args.plots_dir]:
        try:
            os.makedirs(directory, exist_ok=True)
        except PermissionError:
            print(f"❌ 권한 에러: {directory} 생성 실패")
            print("sudo 권한이 필요하거나 다른 경로를 사용하세요")
            return
    ...
    if not ENV_AVAILABLE:
        print("❌ MTD Environment를 로드할 수 없습니다!")
        return
    ...
    try:
        ...
        try:
            if len(trainer.metrics.episode_rewards) > 0:
                ...
                print("📈 중간 결과 그래프 저장 완료")
        except Exception as e:
            print(f"❌ 그래프 생성 실패: {e}")
    
    except Exception as e:
        print(f"❌ Training 중 오류 발생: {e}")
        
        # 오류 발생시에도 현재까지 결과 저장 시도
        try:
            error_path = Path(config.output_dir) / "error_state.pt"
            trainer.agent.save(str(error_path))
            print(f"💾 Error state saved: {error_path}")
        except:
            pass
    ...
    try:
        final_stats = {
            'episodes': len(trainer.metrics.episode_rewards),
            'best_des': trainer.best_des,
            'best_reward': trainer.best_reward,
            'final_des': trainer.metrics.episode_des_scores[-1] if trainer.metrics.episode_des_scores else 0,
            'breach_rate': sum(trainer.metrics.episode_breach_rates) / max(len(trainer.metrics.episode_breach_rates), 1)
        }
        
        print(f"📊 Final Statistics:")
        print(f"  Episodes: {final_stats['episodes']}")
        print(f"  Best DES: {final_stats['best_des']:.3f}")
        print(f"  Best Reward: {final_stats['best_reward']:.1f}")
        print(f"  Final DES: {final_stats['final_des']:.3f}")
        print(f"  Avg Breach Rate: {final_stats['breach_rate']:.3f}")
        
        # 통계 저장
        stats_path = Path(config.output_dir) / "training_stats.json"
