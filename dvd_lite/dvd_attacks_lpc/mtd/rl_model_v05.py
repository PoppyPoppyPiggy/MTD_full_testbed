...
class PPOAgent:
    ...
    def ready_for_update(self):
        return len(self.rewards) >= self.max_buffer_size or (
    ...
    def _prepare_data(self):
        states = torch.as_tensor(np.array(self.states), dtype=torch.float32).to(
            self.device
        )
        actions = torch.as_tensor(np.array(self.actions), dtype=torch.float32).to(
            self.device
        )
        old_log_probs = torch.as_tensor(self.log_probs, dtype=torch.float32).to(
            self.device
        )
        ...
        with torch.no_grad():
            next_state = (
                torch.as_tensor(self.states[-1], dtype=torch.float32)
                .to(self.device)
                .unsqueeze(0)
            )
    ...
    def update_policy(self):
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(0, len(self.rewards), self.minibatch_size):
                ...
                new_log_probs, entropy, new_values = self.network.get_log_prob(
        ...
        with torch.no_grad():
            ...
            var_y = np.var(y_true)
            if var_y > 0:
                explained_variance = 1 - np.var(y_true - y_pred) / (var_y + 1e-8)
            else:
