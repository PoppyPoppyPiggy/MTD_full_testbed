...
try:
    ...
    wandb = None
    print("Warning: 'wandb' a-py (pip install wandb)가 설치되지 않았습니다. --disable-wandb 플래그가 활성화됩니다.")
...
class PPOAgent:
    ...
    def get_action_and_value(self, obs, done_mask):
        ...
        self.value_net.eval()
        with torch.no_grad():
            # 이전 에피소드 종료 시 가치(value)를 0으로 마스킹
            # (GAE 계산 시 중요)
    ...
    def update(self, advantages, returns, b_obs, b_logprobs, b_actions):
        ...
        for epoch in range(self.cfg.n_epochs):
            ...
            for start in range(0, self.cfg.batch_size, self.cfg.minibatch_size):
                ...
                clip_adv = torch.clamp(ratio, 1 - self.cfg.clip_coef, 1 + self.cfg.clip_coef) * mb_advantages
                ...
                # 가치망 업데이트 (손실 재계산)
                # (참고: 일부 구현에서는 Actor/Critic이 파라미터를 공유하지 않으면 V-Loss를 분리하여 계산)
...
def main():
    ...
    # 환경 초기화
    obs = torch.tensor(env.reset(), dtype=torch.float32).to(cfg.device).unsqueeze(0) # (1, 16)
    ...
    for update in range(1, cfg.updates + 1):
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(cfg.batch_size)):
                delta = agent.rewards[t] + cfg.gamma * (next_value if t == cfg.batch_size - 1 else agent.values[t+1]) * (1.0 - agent.dones[t]) - agent.values[t]
                advantages[t] = last_gae_lambda = delta + cfg.gamma * cfg.gae_lambda * (1.0 - agent.dones[t]) * last_gae_lambda
    ...
    # --- 학습 종료 ---
    print("\n학습 완료.")
    env.close() # (참고: 현재 NetworkEnv에는 close()가 없지만, 향후를 위해 추가)
    writer.close()
