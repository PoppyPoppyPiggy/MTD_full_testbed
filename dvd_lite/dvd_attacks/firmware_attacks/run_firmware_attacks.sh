...
main() {
    ...
    # 시뮬레이션 공격 실행
    log_info "펌웨어 공격 시뮬레이션 실행 중..."
    simulation_wait 5 15
    
    add_ioc "$ioc_file" "ATTACK_SIMULATED:firmware_attacks_simulation:$(date +%s)"
    add_ioc "$ioc_file" "TACTIC_COMPLETE:$TACTIC_NAME:$(date +%s)"
    add_ioc "$ioc_file" "SUCCESS_RATE:1/1"
    
    log_success "펌웨어 공격 시뮬레이션 완료"
